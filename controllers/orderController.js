...
exports.createOrder = asyncHandler(async (req, res) => {
  try {
    ...
    if (
      paymentMethod &&
      ["cash", "manual", "bank_transfer"].includes(paymentMethod)
    ) {
...
exports.getOrdersByParty_phoneNumber = async (req, res) => {
  ...
  try {
    const Orgz = await PartyOrganization.find({
...
exports.getOrderById = asyncHandler(async (req, res) => {
  try {
    const order = await WorkOrder.findById(req.params.id).populate(
      "payments",
      "paymentId amount status paymentMethod transactionId paymentDate"
    );
